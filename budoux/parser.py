...
def _get_thai_char_type(char: str) -> str:
  """Returns the Thai character type for the custom boundary features."""
  codepoint = ord(char)
...
class Parser:
  ...
  def parse(self, sentence: str) -> typing.List[str]:
    ...
    for i in range(1, len(sentence)):
      ...
      left_type = _get_thai_char_type(sentence[i - 1])
      right_type = _get_thai_char_type(sentence[i])
      if left_type == 'LV' and right_type == 'C':
        score += self.model.get('LV', {}).get('C', 0)
      if left_type == 'C' and right_type == 'RV':
        score += self.model.get('C', {}).get('RV', 0)
      if left_type == 'C' and right_type == 'AV':
        score += self.model.get('C', {}).get('AV', 0)
