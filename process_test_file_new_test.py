...
def iter_input_lines():
    sources = [
        ("Data/Best/article", "article_{:05d}.txt", 90, 96),
        ("Data/Best/encyclopedia", "encyclopedia_{:05d}.txt", 90, 96),
        ("Data/Best/news", "news_{:05d}.txt", 90, 96),
        ("Data/Best/novel", "novel_{:05d}.txt", 90, 96),
    ]
...
with open("new_test.txt", "w", encoding="utf-8") as wf:
    new_line = ""

    def write_line(chunk: str) -> None:
        chunk = chunk.rstrip("▁").replace("▁", "|")
        if not chunk:
            return
